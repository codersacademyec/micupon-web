...
(function() {
  ...
  configurationVariableNames = (function() {
    function configurationVariableNames(configObjectName) {
      ...
      this.renderEvent = "" + this.configObjectName + ".renderEvent";
    }
  ...
  PaginatedSetup = (function(_super) {
    ...
    PaginatedSetup.prototype.link = function($scope, $element, $attributes, $filter) {
      ...
      getSortedAndPaginatedList = function(list, currentPage, itemsPerPage, orderBy, sortContext, predicate, descending, $filter) {
        ...
        if (list) {
          ...
          if (sortContext === "global") {
            //CAMBIO - para poder ordenar por afuera
            if (predicate) {
      ...
      update = function() {
        ...
        $scope.fillerArray = getFillerArray(w.getList(), w.getCurrentPage(), nop, w.getItemsPerPage());
        //CAMBIO - para poder ejecutar una funcion despues del update de la tabla
        return $scope.$eval("" + w.configurationVariableNames.renderEvent + "()");
      };
